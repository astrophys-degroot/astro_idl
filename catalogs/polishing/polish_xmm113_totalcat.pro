;========================================================================================================================
PRO polish_xmm113_totalcat, FILE1=file1, FILE2=file2, $
                            PRE1=pre1, PRE2=pre2, $
...
;-
  ...
  ;;;default values
  IF keyword_set(FILE1) THEN file1 = file1[0] ELSE file1 = 'xmm113_composite_specz_v2-1.fits'
  ...
  IF keyword_set(FILE2) THEN file2 = file2[0] ELSE file2 = 'xmm113_v4_master.fits'
  IF keyword_set(INDIR2) THEN indir2 = indir2[0] ELSE indir2 = '/Users/adegroot/research/clusters/xmm/xmm113/catalogs/julie/xmm113_catalog_v4/'
  IF keyword_set(PRE2) THEN pre2 = pre2[0] ELSE pre2 = 'ph_'
  IF keyword_set(OUTFILE) THEN outfile = outfile[0] ELSE outfile = 'xmm113_pz_specz_full_v1-0-0.fits'
  ...
  IF chk[0] NE -1 THEN BEGIN
     FOR xx=0UL, n_elements(chk)-1, 1 DO BEGIN
        outstruc[chk[xx]].raone = mean([outstruc[chk[xx]].sp_rabest, outstruc[chk[xx]].ph_ra])
        outstruc[chk[xx]].decone = mean([outstruc[chk[xx]].sp_decbest, outstruc[chk[xx]].ph_dec])
     ENDFOR
  ENDIF
  chk = where((outstruc.sp_rabest NE -99.0) AND (outstruc.sp_decbest NE -99.0) AND $
