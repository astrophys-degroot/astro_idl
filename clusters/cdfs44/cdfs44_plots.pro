...
FUNCTION CMD, xdata, xoptions, RS=rs, SBS=sbs, SUBFOV=subfov
  ...
  IF keyword_set(RS) THEN BEGIN
     ...
     outfile = 'cdfs44_rsfullfov_new.jpeg'
  ENDIF
  IF keyword_set(SBS) THEN BEGIN
     ...
     outfile = 'cdfs44_sbsfullfov_new.jpeg'
...
FUNCTION MYDEDICA, xdata, xoptions
  ...
  theplot = plot(plotvals.xs, plotvals.ys, LINESTYLE=0, /CURRENT, $
                 THICK=2, $
                 TITLE='CDFS44 Adaptive Kernel Reconstruction', $
                 XTITLE='Redshift', $
  ...
  mwrfits, analysis, 'cdfs44_dedica_analysis_v1-0.fits', /CREATE
...
FUNCTION BPT, xdata, xoptions
  ...
  bptplot.save, 'cdfs44_bptall_new.eps', RESOLUTION=600, /TRANSPARENT
...
FUNCTION IRACAGN, xdata, xoptions
  ...
  iracagnplot.save, 'cdfs44_iracagn_new.eps', RESOLUTION=600
...
FUNCTION STACKSPEC, xdata, xoptions
  ...
  specplot1.save, 'cdfs44_stackspec_new.eps', RESOLUTION=600
...
;========================================================================================================================
PRO CDFS44_PLOTS, USEFILE=usefile, YOURFILE=yourfile, AUXFILE=auxfile, $
...
;-
  ...
  options = {FILE:'hithere', AUXFILE:'hithere2', NWIN:1, WEPSILON:750.0, $
             bcgrasex:'03:30:55.88', bcgdecsex:'-28:42:59.9', bcgradeg:0.0D, bcgdecdeg:0.0D, $
             g1rasex:'03:31:05.41', g1decsex:'-28:45:55.5', g1radeg:0.0D, g1decdeg:0.0D, $
             g2rasex:'03:30:48.30', g2decsex:'-28:45:27.8', g2radeg:0.0D, g2decdeg:0.0D, $
             g3rasex:'03:31:20.43', g3decsex:'-28:43:17.4', g3radeg:0.0D, g3decdeg:0.0D, $
             g4rasex:'00:00:00.00', g4decsex:'-00:00:00.0', g4radeg:0.0D, g4decdeg:0.0D, $
             zmincluster:1.610, zmaxcluster:1.642, zming1:1.600, zmaxg1:1.610, $
             pzmincluster:1.426, pzmaxcluster:1.826, $
  ...
  files = ['/Users/adegroot/research/clusters/cdfs/cdfs44/catalogs/CDFS44_pz_specz_full_v3-6-0.fits' , $
           '/Users/adegroot/research/clusters/cdfs/cdfs44/catalogs/julie/cdfs44_pzint_v1-6.fits', $
           '/Users/adegroot/research/clusters/cdfs/cdfs44/catalogs/spectroscopic/cdfs44_composite_specz_v6-2.fits', $
           '/Users/adegroot/research/clusters/cdfs/cdfs44/spectroscopy/stacks/cdfs44_spectra_stack_cat_v3-1.fits', $
           '/Users/adegroot/research/clusters/cdfs/cdfs44/catalogs/spectroscopic/cdfs44_composite_specz_v5-0.fits', $
           '/Users/adegroot/research/clusters/cdfs/cdfs44/auxiliary/cdfs44_dedica_analysis_v1-0.fits'] 
  ...
  IF keyword_set(SPECZHISTALL) THEN BEGIN                  ;if plot desired
     chk = speczhistall(data, options)                     ;call function
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows
  ENDIF                                                    ;end if plot desired

  IF keyword_set(SPECZHISTBCG) THEN BEGIN                  ;if plot desired
     chk = speczhistbcg(data, options)                     ;call function
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows     
  ENDIF                                                    ;end if plot desired

  IF keyword_set(SPECZHISTG1) THEN BEGIN
     chk = speczhistg1(data, options)                      ;call function
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows     
  ENDIF
  ...
  ;;;photo-z overdensity 
  IF keyword_set(PHOTOZOD) THEN BEGIN
     chk = photozod(data, options)                         ;call function     
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows     
  ENDIF


  ;;;red sequence scatter plots
  IF keyword_set(RSFULLFOV) THEN BEGIN                     ;if full FOV red-sequence plot
     chk = cmd(data, options, /RS)                         ;call function     
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows     
  ENDIF

  IF keyword_set(RSSUBFOV) THEN BEGIN                      ;if full FOV red-sequence plot
     chk = cmd(data, options, /RS, SUBFOV=rssubfov)        ;call function     
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows     
  ENDIF


  ;;;stellar bump sequence scatter plots
  IF keyword_set(SBSFULLFOV) THEN BEGIN                    ;if full FOV red-sequence plot
     chk = cmd(data, options, /SBS)                        ;call function     
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows     
  ENDIF
  
  IF keyword_set(SBSSUBFOV) THEN BEGIN                     ;if full FOV red-sequence plot
     chk = cmd(data, options, /SBS, SUBFOV=sbssubfov)      ;call function     
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows     
  ENDIF
  ...
  ;;;DEDICA 
  IF keyword_set(DEDICA) THEN BEGIN                        ;if DEDICA plot is desired
     chk = mydedica(data, options)                         ;call function
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows     
  ENDIF                                                    ;end DEDICA desired

  ;;;CLUSTERIN 
  IF keyword_set(CLUSTERIN) THEN BEGIN                     ;if CLUSTERIN plot is desired
     chk = clusterin(data, auxdata, options)               ;call function
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows     
  ENDIF                                                    ;end CLUSTERIN desired



  ;;;BPT diagram diagnostic
  IF keyword_set(BPT) THEN BEGIN                           ;if BPT plot is desired
     chk = bpt(data, options)                              ;plot BPT 
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows     
  ENDIF


  ;;;IRAC color AGN diagnostic
  IF keyword_set(IRACAGN) THEN BEGIN                       ;if BPT plot is desired
     chk = iracagn(data, options)                          ;plot BPT 
     IF chk EQ 1 THEN print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1                       ;up number of windows     
  ENDIF


  ;;;Mass-metallicty relation single points
  IF keyword_set(MZRINDIV) THEN BEGIN                                                ;if full FOV red-sequence plot
     chk = plot_mzr(data.lmass, 'N2', HAFLUX=data.ha_flux, NIIFLUX=data.niir_flux, $ ;cont next line
                    CLMEM=data.clmem, QUALITY=data.qflag_mosfire)                    ;plot mzr
     IF chk EQ 1 THEN print, '    ...created successfully'                           ;print info
     options.nwin = options.nwin + 1                                                 ;up number of windows     
  ENDIF

  IF keyword_set(MZRSTACK) THEN BEGIN                                               ;if full FOV red-sequence plot
  ...
  IF keyword_set(SPECSTACK) THEN BEGIN ;if full FOV red-sequence plot
     specs = [['../spectroscopy/stacks/cdfs44_spectra_stack_cluster_lowmass_v3-1.fits','../spectroscopy/stacks/cdfs44_spectra_stack_cluster_highmass_v3-1.fits'],$
              ['../spectroscopy/stacks/cdfs44_spectra_stack_field_lowmass_v3-1.fits','../spectroscopy/stacks/cdfs44_spectra_stack_field_highmass_v3-1.fits']]
  ...
  ;;;redshift galaxy density using spec-z's
  IF keyword_set(SPECZDENSITYALL) OR keyword_set(ALL) THEN BEGIN
     print, '  Redshift density of spectroscopically confirmed members...'
     data = mrdfits(file3, 1, hdr)
     zmin = 1.3
     zmax = 1.8
     zbinsz = 0.05
     npts = 10000
     ...
     zmaxnar = 1.7
     zbinsznar = 0.01
     specz = data.speczbest
     chk = where((specz GT 0.0) AND (specz LT 10.0))
     IF chk[0] NE -1 THEN specz = specz[chk]
     especz = specz
     especz[*] = 0.1
     xs = ((zmax-zmin)/npts)*indgen(npts+1) + zmin
     FOR xx=0, n_elements(specz)-1, 1 DO BEGIN
        logtmp = -1.0*(specz[xx]-xs)^2/(2.0*especz[xx]^2)
        IF xx EQ 0 THEN logpdf = logtmp ELSE logpdf = logpdf + logtmp
     ENDFOR
     peak = max(logpdf)
     logpdf = logpdf-peak
     pdf = exp(logpdf)
     w = window(LOCATION=[100+50*nwin,25+25*nwin])
     xplot = plot(xs, pdf, '-', /CURRENT)

     ;phistspecznargood = PLOT(xbinnarrow, gpdfnarrow, /CURRENT, /STAIRSTEP, $
     ;                         /FILL_BACKGROUND, FILL_COLOR='green', $ 
     ;                         /OVERPLOT, $
     ;                         COLOR='green', THICK=2.0)
     print, '    ...created successfully' ;print info
     options.nwin = options.nwin + 1      ;up number of windows     
  ENDIF
