;========================================================================================================================
function sextract_cluster_ch1ch2
...
openw, lun, 'I1I2clustercat_combined.cat', /get_lun
...
sex_path = 'sextractor-2.3.2/src/'
image_path = 'I1I2clustermaps/'
...
if (use_noise eq 'false') then command_line = 'sex ' + strcompress(image_path+'cmap_slice'+string(slicenum)+'.fits', /remove_all) + $
...
if (use_noise eq 'true') then command_line = 'sex ' + strcompress(image_path+'cmap_finalmap'+string(slicenum)+'.fits', /remove_all) + $
' -c ' + image_path+'clusterfind.sex' + ' -WEIGHT_IMAGE ' + strcompress(image_path+'cmap_weightmap'+string(slicenum)+'.fits', /remove_all) + $
' -PARAMETERS_NAME ' + image_path+'default.param' + ' -FILTER_NAME ' + image_path+'default.conv' + ' -CATALOG_NAME ' + $
image_path + 'test.cat'
...
if (use_noise eq 'false') then image = Readfits(strcompress('I1I2clustermaps/cmap_slice'+string(slicenum)+'.fits', /remove_all))
if (use_noise eq 'true') then image = Readfits(strcompress('I1I2clustermaps/cmap_finalmap'+string(slicenum)+'.fits', /remove_all))

readcol, 'I1I2clustermaps/test.cat', cnum, flux_3px, flux_5px, eflux_3px, eflux_5px, flux_auto, eflux_auto,  $
...
openw, lun, 'slice92_image.reg', /GET_LUN
for zz=0, n_elements(xpos)-1, 1 do begin
   printf, lun, xpos[zz], ypos[zz], FORMAT='(2f)'
endfor
free_lun, lun

openw, lun, 'slice92_wcs.reg', /GET_LUN
for zz=0, n_elements(xpos)-1, 1 do begin
   printf, lun, cra[zz], cdec[zz], FORMAT='(2f)'
endfor
free_lun, lun
...
;Remove clusters detected out of bounds - Rare, just in low background, high-z maps
      ...
      for q = 0UL, n_elements(cra) -1 do begin
...
if strpos(strcompress(string(xsc[p]), /remove_all), '.') lt 2 then xxsc[p] = '0'+string(xsc[p]) else xxsc[p] = string(xsc[p])
   ...
   for p = 0UL, n_elements(cra)-1 do begin
...
;========================================================================================================================
pro call_sextract_cluster_ch1ch2
