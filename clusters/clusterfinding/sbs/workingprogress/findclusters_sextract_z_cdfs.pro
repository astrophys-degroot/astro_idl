...
;Read in red sequences from models
  ...
  openw, lun, 'cluster_cat.cat', /get_lun
...
;slicenum = 25
  ...
  for slicenum = startslice, endslice do begin
     ...
     if (use_noise eq 'false') then image = Readfits(strcompress('zclustermaps/cmap_slice'+string(slicenum)+'.fits', /remove_all))
     if (use_noise eq 'true') then image = Readfits(strcompress('zclustermaps/cmap_finalmap'+string(slicenum)+'.fits', /remove_all))
     
     readcol, 'zclustermaps/test.cat', cnum, flux_3px, flux_5px, eflux_3px, eflux_5px, flux_auto, eflux_auto,  $
              xpos, ypos, flags, skipline = 8, format='(i,d,d,d,d,d,d,d,d,i)
     
     if slicenum eq startslice then begin
        window, 0, xsize = 1200, ysize = 1000, title = 'Clustermap' + '   z =' + string(z[slicenum]) + '  Slice: '+  strcompress(string(slicenum) +'/' + string(endslice), /remove_all)
     endif
...
;Read in weights (for getting BCG centroid)
        readweight_final_z_CDFS, ra, dec, zMI1, ezMI1, I1tot, eI1tot, mag_auto, emag_auto, weight, slicenum
...
;Remove clusters detected out of bounds - Rare, just in low background, high-z maps
           ...
           for q = 0, n_elements(cra) -1 do begin
              ...
              temp_mags = I1tot[where( sqrt( ((ra-cra[q])*cos(dec*!pi/180.0))^2 + (dec-cdec[q])^2) lt angsz2)]
