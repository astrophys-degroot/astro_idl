...
openw, lun, '/chooyu1/muzzin/SpARCS/ELAIS-N1/foundclusters_z/testz_sex_cluster_cat.cat', /get_lun
...
image_path = '/chooyu1/muzzin/SpARCS/ELAIS-N1/zclustermaps/'
...
if (use_noise eq 'false') then image = Readfits(strcompress('/chooyu1/muzzin/SpARCS/ELAIS-N1/zclustermaps/cmap_slice'+string(slicenum)+'.fits', /remove_all))
if (use_noise eq 'true') then image = Readfits(strcompress('/chooyu1/muzzin/SpARCS/ELAIS-N1/zclustermaps/cmap_finalmap'+string(slicenum)+'.fits', /remove_all))

readcol, '/chooyu1/muzzin/SpARCS/ELAIS-N1/zclustermaps/test.cat', cnum, flux_3px, flux_5px, eflux_3px, eflux_5px, flux_auto, eflux_auto,  $
xpos, ypos, flags, skipline = 8, format='(i,d,d,d,d,d,d,d,d,i)

window, 0, xsize = 1200, ysize = 1300, title = 'Clustermap' + '   z =' + string(z[slicenum]) + '  Slice: '+  strcompress(string(slicenum) +'/' + string(endslice), /remove_all)
tvscl, image
...
readweight_final_z_ELAISN1, ra, dec, zMI1, ezMI1, I1tot, eI1tot, mag_auto, emag_auto, weight, slicenum
