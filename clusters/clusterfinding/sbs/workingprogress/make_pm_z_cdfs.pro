...
;grid[(fix((ra[x]-minra)/(pixelsize))),(fix((dec[x]-mindec)/(pixelsize)))] = grid[(fix((ra[x]-minra)/(pixelsize))),(fix((dec[x]-mindec)/(pixelsize)))]+ 1.0*weight[x]
        grid[(fix(((ra[x]-minra)*cos(dec[x]*!pi/180.0))/(pixelsize))),(fix((dec[x]-mindec)/(pixelsize)))] = grid[(fix(((ra[x]-minra)*cos(dec[x]*!pi/180.0))/(pixelsize))),(fix((dec[x]-mindec)/(pixelsize)))]+ 1.0*weight[x]
...
;Smooth the image with the kernel
     ...
     window, 1, xsize = 1100, ysize = 900, title = 'Density Map for Z =' + strcompress(string(z[slicenum]), /remove_all)
     tvscl, hind
...
;this computes the density cut necessary to select cluster galaxies
     thresh = 0.05              ; Use 0.05 up to z = 0.85 then switch to 0.02
...
;Compute PM
     ...
     for x = 0L, n_elements(ra)-1 do begin
        if weight[x] gt 0.0 and (I1tot[x] ge xstar[slicenum] - 3.0) and (I1tot[x] le xstar[slicenum] + 4.0) then begin
           ...
           if hind[xpos,ypos] ge cut then begin
              for y = 0, n_elements(NC) - 1 do begin
                 if I1tot[x] ge (xstar[slicenum] - 3.0 + y*0.5) and I1tot[x] le (xstar[slicenum] - 3.0 + (y+1.0)*0.5) then $
                    NC[y] = NC[y]+1.0
              endfor
           endif else begin
...
;device, /close
  ...
  openw, lun, 'zsliceweightszf40/slicepmdata_zother.dat', /get_lun
  printf, lun, header
