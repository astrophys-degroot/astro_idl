...
;Read in the P(M) fitted data
     openr, lun, 'zsliceweightszf40/slicepmdata_zother.dat', /get_lun
     header = strarr(2)
     readf, lun, header
...
;if q eq startslice then window, 2, xsize = 1300, ysize = 700, title = 'Bad Pixel Mask'
     ...
     tvscl, outmap
     writefits, strcompress('cmap_weightmap' + string(slicenum) +'.fits', /remove_all), outmap
;write_jpeg, strcompress('/chooyu1/muzzin/SpARCS/CDFS/zclustermaps/' + 'cmap_weightmap' + string(slicenum) +'.jpg', /remove_all), TVRD()
     
;if q eq startslice then window, 1, xsize = 1300, ysize = 700, title = 'Density Map'
     window, 1, xsize = 1100, ysize = 900, title = 'Density Map'
     tvscl, hind
     writefits, strcompress('cmap_slice' + string(slicenum) +'.fits', /remove_all), hind
;write_jpeg, strcompress('/chooyu1/muzzin/SpARCS/CDFS/zclustermaps/' + 'cmap_slice' + string(slicenum) +'.jpg', /remove_all), TVRD()
