...
openr, lun, 'zsliceweightszf40/slicepmdata_z.dat', /get_lun
header = strarr(2)
readf, lun, header
...
;grid2[(fix(((ra[x]-minra)*cos(dec[x]*!pi/180.0))/(pixelsize))),(fix((dec[x]-mindec)/(pixelsize)))] = grid2[(fix(((ra[x]-minra)*cos(dec[x]*!pi/180.0))/(pixelsize))),(fix((dec[x]-mindec)/(pixelsize)))]+ 1.0
   ...
   if weight[x] gt 0.0 then begin
      if (I1tot[x]-xstar[slicenum]) lt -1.5 then pmweight[x] = (-1.5)*pmslope[slicenum] + pmint[slicenum] else $
         pmweight[x] = (I1tot[x]-xstar[slicenum])*pmslope[slicenum] + pmint[slicenum]
      if pmweight[x] lt 0.0 then begin
         ...
         grid[(fix(((ra[x]-minra)*cos(dec[x]*!pi/180.0))/(pixelsize))),(fix((dec[x]-mindec)/(pixelsize)))] = grid[(fix(((ra[x]-minra)*cos(dec[x]*!pi/180.0))/(pixelsize))),(fix((dec[x]-mindec)/(pixelsize)))]+ 1 
      endif
   endif
endfor
...
window, 1, xsize = 1300, ysize = 1100, title = 'Density Map, z ='+ string(z[slicenum])
tvscl, hind
writefits, strcompress('zclustermaps/' + 'cmap_slice' + string(slicenum) +'.fits', /remove_all), hind
...
window, 2, xsize = 1300, ysize = 1100, title = 'Bad Pixel Mask, z ='+ string(z[slicenum])
tvscl, outmap
writefits, strcompress('zclustermaps/' + 'cmap_weightmap' + string(slicenum) +'.fits', /remove_all), outmap
