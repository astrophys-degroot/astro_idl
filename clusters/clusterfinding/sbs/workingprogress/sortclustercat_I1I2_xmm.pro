...
;Read in clusters
  ...
  nclust = numlines('I1I2clustercat_combined.cat') -2

  openr, lun, 'I1I2clustercat_combined.cat', /get_lun
  header = strarr(2)
  readf, lun, header
...
;Open the logfile
  openw, lun5, 'catmerge.log', /get_lun
  clustersremain = 'true'
  debug = 0
  ...
  while clustersremain eq 'true' and debug lt 1000 do begin
     ...
     test = n_elements(wz)	
     wz = [wz,zholder[where(fluxholder[*] eq useless)]] 
     wsra = [wsra, sraholder[where(fluxholder[*] eq useless)]] 
     wsdec = [wsdec, sdecholder[where(fluxholder[*] eq useless)]]
     wxpos = [wxpos, xposholder[where(fluxholder[*] eq useless)]] 
     wypos = [wypos, yposholder[where(fluxholder[*] eq useless)]] 
     wra = [wra, raholder[where(fluxholder[*] eq useless)]]
     wdec = [wdec, decholder[where(fluxholder[*] eq useless)]] 
     wflux =[wflux, fluxholder[where(fluxholder[*] eq useless)]] 
     wslicenum = [wslicenum, slicenumholder[where(fluxholder[*] eq useless)]]
     test2 = n_elements(wz)
...
;Write out the data file
  openw, lun, 'cdfs_clustercat_v1.0.0.cat', /get_lun
  printf, lun, header
  for x = 1UL, n_elements(wdec) - 1.0 do begin
