...
function catbase
  ...
  ;           MOBINSLIT:0}
  catbase = {rah:0, ram:0, ras:0.0, decd:0, decm:0, decs:0.0}
...
FUNCTION addredshift, xcat, xheader, xx, ZTAG=ztag, DZTAG=dztag, QFLAGTAG=qflagtag
  ...
  IF keyword_set(ZTAG) THEN ztag = ztag[0] ELSE ztag = 'ZANZMLE'                ;set new value
  IF keyword_set(DZTAG) THEN dztag = dztag[0] ELSE dztag = 'ZANDZMLE'           ;set new value
...
FUNCTION addfluxes, xcat, xheader, xx, xfluxes, $
  ...
  FOR ww=0, n_elements(xfluxes)-1, 1 DO BEGIN ;loop over fluxes
     ...
     strreplace, tmptag, '???', xfluxes[ww]                          ;replace 
     tmptag = strcompress(tmptag, /REMOVE_ALL)                       ;remove whitespace
     fval = fxpar(xheader, tmptag)                                    ;get value
     IF fval EQ 0 THEN xcat[xx].(ind) = -99 ELSE xcat[xx].(ind) = fval ;get value from header
     ...
     chk = tag_exist(xcat, tmptag, INDEX=ind, /TOP_LEVEL)                                    ;find tag index
     tmptag = hflagtag                                                                       ;copy value
     strreplace, tmptag, '???', xfluxes[ww]                                                  ;replace 
     tmptag = strcompress(tmptag, /REMOVE_ALL)                                               ;remove whitespace
     lval = fxpar(xheader, tmptag)                                                           ;get value
     IF ((lval EQ 0) AND (fval EQ 0)) THEN xcat[xx].(ind) = -99 ELSE xcat[xx].(ind) = lval   ;get value from header
...
;-    
  ...
  IF keyword_set(MASKDIR) THEN maskdir = maskdir[0] ELSE maskdir = ' '                             ;set new value
  ...
  IF keyword_set(SPECVER) THEN specver = specver[0] ELSE specver = 'v1-2'                           ;sets new value
  ...
  IF verbose GE 1 THEN BEGIN                                 ;check verbose value
     ...
     print, ' Mask name: ', xmask                            ;print info
     print, '**********************************************' ;print info
  ENDIF                                                      ;end check verbose value


  ;;;create list of 1D spectra
  list1d = strcompress(xmask + '_1dspeclist_' + specver + '.txt', /REMOVE_ALL) ;create name for file
  print, 'ls ' + xmask + '*eps*' + specver + '*.fits > '+ list1d
  spawn, 'ls ' + xmask + '*eps*' + specver + '*.fits > '+ list1d               ;populate file
  ...
  bit = ' '                     ;intialize
  nspec = file_lines(list1d)    ;find number of spectra
  print, list1d
