...
;       A. DeGroot, 2014 Jan 22, Univ. Cali, Riverside v2.0.0
;          -converted to OOP
...
;====================================================================================================
PRO mosfire_analyze1d::getprop, OBJVER=objver, DATAHDR=datahdr, INITGSS=initgss, FMCRUNS=fmcruns

  IF arg_present(OBJVER) THEN objver = self.maoobjver  ;return the data
...
;====================================================================================================
PRO mosfire_analyze1d::prep, mciters, BASENAME=basename, FITTO=fitto, FITTOWL=fittowl, $
  ...
  self.epsfiletd = strcompress(self.basename + '_' + self.object + '_eps.fits', /REMOVE_ALL)                                              ;make new name
  self.mepsfiletd = strcompress(self.basename + '_' + self.object + '_eps_masked.fits', /REMOVE_ALL)                                      ;make new name
  self.sigfiletd = strcompress(self.basename + '_' + self.object + '_sig.fits', /REMOVE_ALL)                                              ;make new name
  self.snrfiletd = strcompress(self.basename + '_' + self.object + '_snrs.fits', /REMOVE_ALL)                                             ;make new name
  self.msnrfiletd = strcompress(self.basename + '_' + self.object + '_snrs_masked.fits', /REMOVE_ALL)                                     ;make new name
  self.waveresid = strcompress(self.basename + '_applywavefit.txt', /REMOVE_ALL)                                                          ;make new name
  self.epsfileod = strcompress(self.basename + '_' + self.object + '_eps_1d_' + oldver + '.fits', /REMOVE_ALL)                            ;make new name
  self.mcruns = strcompress(self.basename + '_' + self.object + '_eps_1d_' + oldver + '_mcruns' + string(mciters) + '.fits', /REMOVE_ALL) ;make new name
  self.mosaicfile = strcompress(self.basename + '_' + self.object + '_evid.tiff', /REMOVE_ALL)                                            ;make new name
  self.fitfile = strcompress(self.basename + '_' + self.object + '_eps_1d_' + oldver + '_lines.ps', /REMOVE_ALL)                          ;make new name
  self.fitdistfile = strcompress(self.basename + '_' + self.object + '_eps_1d_' + oldver + '_linesdist.ps', /REMOVE_ALL)                  ;make new name
  self.graphoutfile = strcompress(self.basename + '_' + self.object + '_eps_1d_' + oldver + '_linessky.ps', /REMOVE_ALL)                  ;make new name
  ...
  print, '   2D spectrum signal file name: ', self.epsfiletd
  print, '   2D spectrum sigma file name: ', self.sigfiletd
  print, '   2D spectrum SNR file name: ', self.snrfiletd
  print, '   2D spectrum SNR masked file name: ', self.msnrfiletd
  print, '   Wavelength calibration residual file: ', self.waveresid
...
;====================================================================================================
FUNCTION mosfire_analyze1d::prepmcdata, xdata, xmciters, MCDATA=mcdata, NXS=nxs
...
;====================================================================================================
FUNCTION mosfire_analyze1d::prepmcguess, xmciters, NXS=nxs, DZ=dz, OTHFRAC=othfrac
  ...
  ;;;structures to fill
  fguess = self.initialguess::decipher(*self.fitto, self.border) ;decipher text string 
...
;====================================================================================================
FUNCTION mosfire_analyze1d::findfiles, INDIR=indir, OUTDIR=outdir
...
;====================================================================================================
FUNCTION mosfire_analyze1d::readfiles, xwhich, ERRFAC=errfac
...
;====================================================================================================
FUNCTION mosfire_analyze1d::getsub, xdata, xflg
...
;====================================================================================================
PRO mosfire_analyze1d::bounds, LAL=lal, LAU=lau, YL=yl, YU=yu
...
;====================================================================================================
FUNCTION mosfire_analyze1d::lambsubset, NOFITSKY=nofitsky, KEYSKY=keysky
...
;====================================================================================================
FUNCTION mosfire_analyze1d::fitspec, xdata, NEWDATA=newdata, $
  ...
  IF keyword_set(GETVARY) THEN BEGIN                                                      ;just want step size
     ...
     bayesian_spectral, xdata.lambdas, fitdata, xdata.spec1dwei, $                        ;cont next line
...
;====================================================================================================
FUNCTION mosfire_analyze1d::zbrkdwn, xfitvals, MINERR=minerr, RESID=resid, WVRESID=wvresid

  COMPILE_OPT IDL2              ;force settings

  IF keyword_set(MINERR) THEN minerr = float(minerr[0]) ELSE minerr = 0.000001               ;set default value 
  IF keyword_set(WVRESID) THEN wvresid = float(wvresid[0]) ELSE wvresid = xfitvals[-1].medxs ;set default value 
  
  
  ;;;redshift output
  zs = xfitvals.redshift                              ;get redshift values
  dzs = 0.5*(xfitvals.redshiftm + xfitvals.redshiftp) ;get average redshift errors
  
  ;;;find MLE error; we use biweight estimate to remove effect of catastrophic failures
  mymledz = obj_new('beers1990', dzs) ;new object
  mledz = mymledz.biweight()          ;find biweight estimate
  obj_destroy, mymledz                ;destroy object

  ;;;find MC value and error
  mymcz = obj_new('beers1990', zs) ;new object
  mcz = mymcz.biweight()           ;find biweight estimate
  obj_destroy, mymcz               ;destroy object
  mcdz = sigma(zs)                 ;std dev of redshifts

  ;;;find systematic error from width of line
  mysysdzline = obj_new('beers1990', xfitvals.sig1) ;new object
  linewid = mysysdzline.biweight()                  ;find biweight estimate
  sysdzline = linewid / wvresid                     ;redshift error
  obj_destroy, mysysdzline                          ;destroy object

  ;;;residual wavelength fitting error
  IF keyword_set(resid) THEN BEGIN             ;if the file exists
     chk = where(resid.object EQ self.object)  ;find object in structure
     IF (chk[0] NE -1) THEN BEGIN              ;if found
        syswvdz = resid[chk].resrms / wvresid  ;grab eror
     ENDIF ELSE BEGIN                          ;if not found
        syswvdz = mean(resid.resrms) / wvresid ;take average error
     ENDELSE                                   ;end not found
  ENDIF ELSE syswvdz = 0.0                     ;end if the file exists

  ;;;total dz
  bit1 = string(zs[-1], FORMAT='(F8.6)')                                             ;format
  bit2 = string(mcz, FORMAT='(F0.6)')                                                ;format 
  totdzmle = (minerr^2 + syswvdz^2 + mledz^2)^0.5                                    ;add mle errors in quadrature
  totdzmc = (minerr^2 + syswvdz^2 + mcdz^2)^0.5                                      ;add mc errors in quadrature
  bit3 = string(totdzmle, FORMAT='(F0.6)')                                           ;format 
  bit4 = string(totdzmc, FORMAT='(F0.6)')                                            ;format 
  outline = strcompress(' MLE REDSHIFT: ' + bit1 + ' +/- ' + bit3)                   ;line to print
  print, outline                                                                     ;print info
  print, '   The error comes from adding in quadrature the following: '              ;print info
  print, '     Minimum error: ', minerr                                              ;print info
  print, '     Systematic error from wavelength solution fitting: ', syswvdz         ;print info
  print, '     MLE error using the original spectrum fit of redshift: ', dzs[-1]     ;print info
  print, '     OR MLE (biweight) error using all spectrum fits of redshift: ', mledz ;print info
  outline = strcompress(' MC REDSHIFT: ' + bit2 + ' +/- ' + bit4)                    ;line to print
  print, outline                                                                     ;print 
  print, '   The error comes from adding in quadrature the following: '              ;print info
  print, '     Minimum error: ', minerr                                              ;print info
  print, '     Systematic error from wavelength solution fitting: ', syswvdz         ;print info
  print, '     MC error from std dev of all redshifts: ', mcdz                       ;print info
  print, '   Sanity check: error should be approximately: ', sysdzline               ;print info


  RETURN, {zmle:bit1, dzmle:bit3, zmc:bit2, dzmc:bit4}
END
;====================================================================================================
...
;====================================================================================================
FUNCTION mosfire_analyze1d::fitbrkdwn, xfitvals, xborder, WLINERR=wlinerr
...
;====================================================================================================
FUNCTION mosfire_analyze1d::flagging, xzs, xfitbrk, NSIGNEG=nsigneg, TAGNOSKY=tagnosky
...
;====================================================================================================
FUNCTION mosfire_analyze1d::updatehdr, whichupdate, FITVALS=fitvals, ZVALS=zvals, FITBRK=fitbrk, $
                                       FLGVALS=flgvals
  ...
  FOR xx=0, n_elements(whichupdate)-1, 1 DO BEGIN ;loop over header update
     CASE whichupdate[xx] OF                      ;which header update
        ...
        'basic' : BEGIN                                                                                   ;the basic info
           ...
           fxaddpar, xdatahdr, 'VZAN', self.maoobjver, 'Version of redshift analysis code'                ;add to header
...
;====================================================================================================
FUNCTION mosfire_analyze1d::writefile
...
;====================================================================================================
PRO mosfire_analyze1d::displaytwod, xmybase, LAL=lal, LAU=lau, OPENFITS=openfits


  IF keyword_set(LAL) THEN lal = lal[0] ELSE lal = self.lal ;default value 
  IF keyword_set(LAU) THEN lau = lau[0] ELSE lau = self.lau ;default value 
  xcent = mean([lal, lau])                                  ;center x position
  outname = self.mosaicfile                                 ;graphic name
  
  ;;;Open images for evaluation
  IF keyword_set(OPENFITS) THEN BEGIN                                                ;if OPENFITS keyword present
     line = 'ds9 -geometry 1280x720 -tile row '                                      ;part of ds9 call 
     line = line + '-zscale -zoom 2 ' + self.epsfiletd                               ;part of ds9 call
     line = line + ' -zscale ' + self.mepsfiletd                                     ;part of ds9 call
     line = line + ' -zscale ' + self.snrfiletd                                      ;part of ds9 call
     line = line + ' -frame new -minmax -scale sqrt ' + self.msnrfiletd              ;part of ds9 call
     line = line + ' -frame 1 -pan to ' + string(xcent) + ' 4 wcs -match frame wcs ' ;part of ds9 call
     line = line + '-saveimage tiff ' + outname + ' none &'                          ;part of ds9 call
...
;====================================================================================================
PRO mosfire_analyze1d::displayoned, xmylambflg, NEWDATA=newdata ;, SPEC=spec
...
;====================================================================================================
PRO mosfire_analyze1d::displayonedout, xzz, NEWHDR=newhdr
  ...
  spectra_1d, *self.data, datahdr, ZZ=float(xzz), YERR=self.yerr, $                ;plot analysis
...
;====================================================================================================
PRO mosfire_analyze1d::run, ymciters
...
;====================================================================================================
FUNCTION mosfire_analyze1d::init, xbasename, xobj, $
                                  ...
                                  ZQFLAG=zqflag, KEYSKY=keysky, YERR=yerr, $
                                  INDIR=indir, OUTDIR=outdir, VERBOSE=verbose, HELP=help
                                  
  

  self.maoobjver = 'v2-5-1'                                                    ;code version
  ...
  IF keyword_set(INITGSS) THEN BEGIN
     self.initgss = ptr_new(initgss)                                                      ;store info
     status = self.initialguess::init(initgss)                                            ;initialize 
  ENDIF ELSE $                                                                            ;cont next line
     self.initgss = ['redshift=1.626', 'sig=3.0', 'c1=1.0', 'x0=0.1', 'x1=0.0001' ]       ;sets new value
  IF keyword_set(PRIORS) THEN self.priors = ptr_new(priors) ELSE $                        ;cont next line
     self.priors = ['REDSHIFT GT 1.642', 'REDSHIFT LT 1.652', 'SIG1 GT 0.0', 'C1 GT 0.0'] ;sets new value
  IF keyword_set(FITTO) THEN self.fitto = ptr_new(fitto) ELSE self.fitto = ['Ha']         ;sets new value
  ...
  IF keyword_set(YERR) THEN self.yerr = fix(yerr[0]) ELSE self.yerr = 1                   ;sets new value
...
;====================================================================================================
PRO mosfire_analyze1d__define

  void = {mosfire_analyze1d, INHERITS initialguess, maoobjver:'A', basename:'A', object:'A', $
          epsfiletd:'A', mepsfiletd:'A', sigfiletd:'A', snrfiletd:'A', msnrfiletd:'A', waveresid:'A', $
          epsfileod:'A', mcruns:'A', mosaicfile:'A', fitfile:'A', fitdistfile:'A', graphoutfile:'A', $
          data:ptr_new(), datahdr:ptr_new(), lambflg:ptr_new(), $
          dryrun:0, nofitsky:0, initgss:ptr_new(), priors:ptr_new(), $
