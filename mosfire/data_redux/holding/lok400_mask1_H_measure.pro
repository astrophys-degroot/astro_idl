...
;========================================================================================================================
pro lok400_mask1_H_measure, LIST=list, MASK=mask, OUTFILE=outfile, NAMEPOS=namepos, $
                            SGNFCUT=SGNFCUT, $
  ...
  IF keyword_set(LIST) THEN list = list[0] ELSE list = 'lok400_mask1_H_1dspeclist_v1-1.txt'
  IF keyword_set(LISTDIR) THEN listdir = listdir[0] ELSE $
     listdir = '/Users/degroota/research/current1_mosfire_2013nov28_lok400/lok400_mask1_H/'
  IF keyword_set(MASK) THEN mask = mask[0] ELSE mask = 'lok400_mask1_orig.coords'
  IF keyword_set(MASKDIR) THEN maskdir = maskdir[0] ELSE $
     maskdir = '/Users/degroota/research/observing/keck/mosfire/2013nov28/lok400/lok400_mask1/'
  IF keyword_set(NAMEPOS) THEN namepos = namepos[0] ELSE namepos = 3
  IF keyword_set(OUTFILE) THEN outfile = outfile[0] ELSE outfile = 'lok400_mask1_v4-0-0.fits'
  ...
  FOR xx=0, nlines-1, 1 DO BEGIN                                        ;loop over spectra
     ...
     redshift = fxpar(hdr, 'Z')                                         ;get redshift
     IF redshift NE -99 THEN BEGIN                                      ;check if redshift exists
