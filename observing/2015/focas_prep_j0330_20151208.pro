;========================================================================================================================
pro j0330_make_focas_inputcat, HELP=help
  ...
  datain = '/Users/adegroot/research/clusters/cdfs/cdfs44/catalogs/seechange/SPARCSJ0330_HSTv5_cdfs44v3-9-1_v1-0_all.fits'
  starsin = '/Users/adegroot/research/clusters/cdfs/cdfs44/catalogs/public/cdfs44_2mass_JHK_14arcmin_v1-1.fits'
  catout = 'j0330_feedtofocas_formask1_v1-1.txt'
  ...
  FOR xx=0, n_elements(data.(0))-1, 1 DO BEGIN
     IF (data[xx].mra NE -99.) THEN data[xx].ra = data[xx].mra ELSE data[xx].ra = data[xx].adjustra
     IF (data[xx].mdec NE -99.) THEN data[xx].dec = data[xx].mdec ELSE data[xx].dec = data[xx].adjustdec
  ENDFOR
  ...
  specz = [0.0, 1.606, 1.646, 3.0]          ;spectroscopic redshift bins
  qspecz = [0, 1, 2, 3]                     ;spectroscopic redshift quality bins
  zphot = [0.0, 1.0, 1.426, 1.826, 2.044]   ;distance divisions [arcmin]
  qzphot = [0.0, 0.25, 0.50, 0.75, 1.0]     ;distance divisions [arcmin]
  lsfr = [-2.0,0.0,0.477,10.0]              ;star formation rate
  zflux = [1.58,3.98,3981.0,25118.9]        ;z band flux
  ...
  mosfire_magmacat_specz_photoz, data, stars, DATAHDR=datahdr, STARHDR=starhdr, SUBSETS=subsets, $
                                 ...
                                 TAGSTRA='RAJ2000', TAGSTDEC='DEJ2000', TAGSTJ='Jmag', $
